...
(function () {
  ...
  // ─── Autocomplete de Despachos ─────────────────────────────
  
  var checkDespachoExato = null;

  function initDespachoAutocomplete() {
    var inputDespacho = document.getElementById('rpi-field-despacho');
    var inputAtivo = document.getElementById('rpi-field-ativo');
    var listbox = document.getElementById('rpi-despacho-listbox');
    var errorMsg = document.getElementById('rpi-search-error');
    if (!inputDespacho || !inputAtivo || !listbox) return;

    var activeIndex = -1;
    var currentSuggestions = [];
    var despachoValidado = null;

    function normalize(text) {
      return removeAccents(text).toLowerCase().trim();
    }

    function renderList(suggestions, message) {
      listbox.innerHTML = '';
      if (message) {
        var li = document.createElement('li');
        li.className = 'rpi-autocomplete-message';
        li.textContent = message;
        listbox.appendChild(li);
        listbox.hidden = false;
        inputDespacho.setAttribute('aria-expanded', 'true');
        return;
      }
      
      if (suggestions.length === 0) {
        listbox.hidden = true;
        inputDespacho.setAttribute('aria-expanded', 'false');
        return;
      }

      suggestions.forEach(function(s, index) {
        var li = document.createElement('li');
        li.className = 'rpi-autocomplete-option';
        li.setAttribute('role', 'option');
        li.setAttribute('id', 'rpi-despacho-opt-' + index);
        li.textContent = s.codigo + ' — ' + s.descricao;
        
        // Usar mousedown para prevenir perda de foco no input
    ...
    function selectSuggestion(s) {
      despachoValidado = s;
      inputDespacho.value = s.codigo + ' — ' + s.descricao;
      closeList();
      inputDespacho.focus();
    }
    ...
    function updateActive() {
      ...
      options.forEach(function(opt, idx) {
        if (idx === activeIndex) {
          ...
          inputDespacho.setAttribute('aria-activedescendant', opt.id);
      ...
      if (activeIndex === -1) {
        inputDespacho.removeAttribute('aria-activedescendant');
      }
    }

    function doSearch() {
      var val = inputDespacho.value.trim();
      var ativo = inputAtivo.value;
      
      var minLength = /^\d/.test(val) ? 1 : 3;
      ...
      if (!ativo) {
        renderList([], 'Selecione primeiro um tipo de ativo.');
        return;
      }

      var source = (window.RPI_DESPACHOS && window.RPI_DESPACHOS[ativo]) || [];
      var normVal = normalize(val);

      currentSuggestions = source.filter(function(s) {
        return normalize(s.codigo).indexOf(normVal) > -1 || normalize(s.descricao).indexOf(normVal) > -1;
      });

      if (currentSuggestions.length === 0) {
        renderList([], 'Nenhum despacho encontrado para este tipo de ativo.');
      } else {
        activeIndex = -1;
    ...
    inputDespacho.addEventListener('input', function() {
      despachoValidado = null; // Invalida imediatamente ao editar
      if (errorMsg && errorMsg.getAttribute('aria-hidden') === 'false') {
        errorMsg.setAttribute('aria-hidden', 'true');
      }
      doSearch();
    });

    inputDespacho.addEventListener('focus', function() {
      if (!listbox.hidden) return;
      var val = inputDespacho.value.trim();
      var minLength = /^\d/.test(val) ? 1 : 3;
      if (val.length >= minLength && !despachoValidado) {
        doSearch();
      }
    });

    inputDespacho.addEventListener('keydown', function(e) {
      if (listbox.hidden && e.key !== 'Tab') {
        if (e.key === 'ArrowDown') doSearch();
        return;
      }
    ...
    // Fechar e validar ao perder o foco (blur ou clique externo)
    document.addEventListener('mousedown', function(e) {
      if (!inputDespacho.contains(e.target) && !listbox.contains(e.target)) {
        if (!listbox.hidden) {
          closeList();
          checkExactMatch();
        }
    ...
    inputDespacho.addEventListener('focusout', function(e) {
      // Verifica se o foco foi para a lista (nao deveria devido ao e.preventDefault do mousedown)
      if (e.relatedTarget && listbox.contains(e.relatedTarget)) return;
      closeList();
      checkExactMatch();
    });

    inputAtivo.addEventListener('change', function() {
      inputDespacho.value = '';
      despachoValidado = null;
      closeList();
    });

    checkDespachoExato = function() {
      var val = inputDespacho.value.trim();
      if (!val) return true;
      if (despachoValidado && inputDespacho.value === despachoValidado.codigo + ' — ' + despachoValidado.descricao) {
        return true;
      }
      var ativo = inputAtivo.value;
      ...
      var exacts = source.filter(function(s) {
        var comp = s.codigo + ' — ' + s.descricao;
        return normalize(s.codigo) === normVal || 
               normalize(s.descricao) === normVal || 
               normalize(comp) === normVal;
      });

      if (exacts.length === 1) {
        selectSuggestion(exacts[0]);
        return true;
      }
      return false;
    };
  ...
  // ─── Modal de Referência de Despachos ───────────────────────

  function initDespachoReferenceModal() {
    var btnInfo = document.getElementById('rpi-btn-despachos-info');
    var modal = document.getElementById('rpi-modal-despachos');
    if (!btnInfo || !modal) return;

    var btnClose = document.getElementById('rpi-btn-close-despachos');
    var overlay = modal.querySelector('.rpi-despachos-modal-overlay');
    var navList = document.getElementById('rpi-despachos-nav-list');
    var sectionsContainer = document.getElementById('rpi-despachos-sections');
    var inputAtivo = document.getElementById('rpi-field-ativo');
    var data = window.RPI_DESPACHOS || {};
    var isBuilt = false;

    // Normaliza o nome do ativo para cruzamento (ignora o sufixo "em breve")
    function normalizeAtivoName(name) {
      return (name || '').replace(' — em breve', '').trim();
    }

    // Constrói o DOM do modal baseado no rpi-despachos.js
    function buildModalContent() {
      ...
      for (var key in data) {
        ...
        var despachos = data[key];
        if (!despachos || despachos.length === 0) continue;
        
        var cleanName = normalizeAtivoName(key);
        var secId = 'rpi-sec-' + index;

        // Nav item
        navHtml += '<li>';
        navHtml += '<button type="button" class="rpi-despachos-nav-btn" data-target="' + secId + '" data-key="' + key + '">';
        navHtml += cleanName;
        navHtml += '</button>';
        navHtml += '</li>';

        // Section item
        sectionsHtml += '<section class="rpi-despachos-section" id="' + secId + '">';
        sectionsHtml += '<h3>' + cleanName + '</h3>';
        sectionsHtml += '<ul class="rpi-despachos-list">';
        for (var i = 0; i < despachos.length; i++) {
          var d = despachos[i];
          sectionsHtml += '<li><strong>' + d.codigo + '</strong> — ' + d.descricao + '</li>';
        }
        sectionsHtml += '</ul>';
      ...
      var navBtns = navList.querySelectorAll('.rpi-despachos-nav-btn');
      navBtns.forEach(function(btn) {
        btn.addEventListener('click', function() {
          var targetId = this.getAttribute('data-target');
          var targetEl = document.getElementById(targetId);
          if (targetEl) {
            // Usa scrollTop no container para evitar que a página inteira ao fundo role
            sectionsContainer.scrollTop = targetEl.offsetTop - sectionsContainer.offsetTop;
      ...
      if ('IntersectionObserver' in window) {
        var sections = sectionsContainer.querySelectorAll('.rpi-despachos-section');
        var observerOptions = {
    ...
    function updateActiveNav(secId) {
      var navBtns = navList.querySelectorAll('.rpi-despachos-nav-btn');
      navBtns.forEach(function(btn) {
        if (btn.getAttribute('data-target') === secId) {
          btn.classList.add('rpi-active');
        } else {
          btn.classList.remove('rpi-active');
        }
      });
    }
    ...
    function openModal() {
      ...
      // Abertura Contextual
      var currentAtivo = inputAtivo ? inputAtivo.value : '';
      var scrolled = false;
      
      if (currentAtivo) {
        var navBtns = navList.querySelectorAll('.rpi-despachos-nav-btn');
        for (var i = 0; i < navBtns.length; i++) {
          if (normalizeAtivoName(navBtns[i].getAttribute('data-key')) === normalizeAtivoName(currentAtivo)) {
            var targetId = navBtns[i].getAttribute('data-target');
            var targetEl = document.getElementById(targetId);
            if (targetEl) {
              sectionsContainer.scrollTop = targetEl.offsetTop - sectionsContainer.offsetTop;
              updateActiveNav(targetId);
      ...
      if (!scrolled) {
        ...
        var firstBtn = navList.querySelector('.rpi-despachos-nav-btn');
        if (firstBtn) {
          updateActiveNav(firstBtn.getAttribute('data-target'));
        }
      }

      // Foco inicial
      var title = document.getElementById('rpi-modal-despachos-title');
      if (title) {
    ...
    function handleFocusTrap(e) {
      ...
      var focusableEls = modal.querySelectorAll('a[href], button:not([disabled]), textarea:not([disabled]), input[type="text"]:not([disabled]), input[type="radio"]:not([disabled]), input[type="checkbox"]:not([disabled]), select:not([disabled]), [tabindex]:not([tabindex="-1"])');
      var firstFocusable = focusableEls[0];
      var lastFocusable = focusableEls[focusableEls.length - 1];

      if (e.key === 'Tab') {
        if (e.shiftKey) { // Shift + Tab
          if (document.activeElement === firstFocusable || document.activeElement === document.getElementById('rpi-modal-despachos-title')) {
            e.preventDefault();
            lastFocusable.focus();
          }
        } else { // Tab
  ...
  function initIndexPage() {
    initDespachoAutocomplete();
    initDespachoReferenceModal();
    ...
    searchForm.addEventListener('submit', function (e) {
      ...
      if (!ativo && !numero && !partes && !despacho && !dataInicial && !dataFinal) {
        errorMsg.innerHTML = '<i class="fas fa-exclamation-circle" aria-hidden="true"></i> Preencha pelo menos um critério de pesquisa.';
        errorMsg.setAttribute('aria-hidden', 'false');
        errorMsg.focus();
        return;
      }

      if (checkDespachoExato) {
        var isValid = checkDespachoExato();
        despacho = document.getElementById('rpi-field-despacho').value.trim(); // re-ler valor caso tenha auto-completado
        
        if (despacho && !isValid) {
          var source = (window.RPI_DESPACHOS && window.RPI_DESPACHOS[ativo]) || [];
          var normVal = removeAccents(despacho).toLowerCase();
          var isFound = source.some(function(s) {
            return removeAccents(s.codigo).toLowerCase().indexOf(normVal) > -1 ||
                   removeAccents(s.descricao).toLowerCase().indexOf(normVal) > -1;
          });
          
          if (!isFound) {
            errorMsg.innerHTML = '<i class="fas fa-exclamation-circle" aria-hidden="true"></i> Nenhum despacho encontrado para o tipo de ativo selecionado.';
          } else {
            errorMsg.innerHTML = '<i class="fas fa-exclamation-circle" aria-hidden="true"></i> Selecione um despacho válido na lista de sugestões.';
          }
          errorMsg.setAttribute('aria-hidden', 'false');
          errorMsg.focus();
          return;
